...
class Pipeline:
    ...
    def _create_source_bin(self, index=0):
        ...
        uri_decode_bin = self._create_element(
            "uridecodebin", "uri-decode-bin", "URI decode bin", add=False
        )
    ...
    def _create_mp4_sink_bin(self):
        ...
        nvvidconv3 = self._create_element(
            "nvvideoconvert", "convertor3", "Converter 3", add=False
        )
        capsfilter2 = self._create_element(
        ...
        self._link_sequential(
            [nvvidconv3, capsfilter2, encoder, codeparser, container, filesink]
        )
    ...
    def _create_rtsp_sink_bin(self):
        ...
        # Make the encoder
        encoder = self._create_element(
            f"nvv4l2{self.rtsp_codec.lower()}enc",
            "encoder",
        ...
        rtppay = self._create_element(
            f"rtp{self.rtsp_codec.lower()}pay",
            "rtppay",
    ...
    def _create_elements(self):
        ...
        self.nvvidconv1 = self._create_element(
            "nvvideoconvert", "convertor1", "Converter 1"
        )
        self.capsfilter1 = self._create_element(
            "capsfilter", "capsfilter1", "Caps filter 1"
        )
        self.capsfilter1.set_property(
            "caps", Gst.Caps.from_string("video/x-raw(memory:NVMM), format=RGBA")
        )
        self.tiler = self._create_tiler()
        self.nvvidconv2 = self._create_element(
            "nvvideoconvert", "convertor2", "Converter 2"
        )
    ...
    def _save_crops(self, frames, _, l_frame_meta: List, ll_obj_meta: List[List]):
        ...
        for frame, frame_meta, l_obj_meta in zip(frames, l_frame_meta, ll_obj_meta):
            ...
            for obj_meta in l_obj_meta:
                ...
                if not self.track_scores[track_id] or crop_score > max(
                    self.track_scores[track_id]
                ):
                    crop_dir = os.path.join(
                        CROPS_DIR,
                        f"src_{frame_meta.source_id}",
                    ...
                    crop_path = os.path.join(
                        crop_dir, f"frame_{frame_meta.frame_num}.jpg"
                    )
    ...
    def _add_probes(self):
        ...
        if self.enable_osd and self.write_osd_analytics:
            tiler_sinkpad.add_probe(
                Gst.PadProbeType.BUFFER, self._wrap_probe(self._write_osd_analytics)
            )

        if self.save_crops:
            tiler_sinkpad.add_probe(
                Gst.PadProbeType.BUFFER, self._wrap_probe(self._save_crops)
            )
